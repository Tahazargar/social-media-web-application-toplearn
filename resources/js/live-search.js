...
export default class Search {
  ...
  renderResultsHTML(posts) {
    if (posts.length) {
      ...
      <div class="list-group-item active"><strong>نتایج جستجو</strong> (${posts.length > 1 ? `${posts.length} نتیجه پیدا شد` : "1 نتیجه پیدا شد"})</div>
      ${posts
        .map(post => {
        ...
        <span class="text-muted small">توسط ${post.user.username} در ${postDate.getMonth() + 1}/${postDate.getDate()}/${postDate.getFullYear()}</span>
      </a>`
        })
        .join("")}
    </div>`)
    } else {
      this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">متاسفانه نتیجه ای برای نمایش یافت نشد!</p>`
    }
    this.hideLoaderIcon()
  ...
  injectHTML() {
    ...
    <div class="search-overlay-top shadow-sm">
      <div class="container container--narrow">
        ...
        <input autocomplete="off" type="text" id="live-search-field" class="live-search-field" placeholder="دنبال چه می‌گردید؟">
